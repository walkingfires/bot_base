...
class BaseDAO(Generic[T]):
    ...
    @classmethod
    async def find_all_by_field(cls, session: AsyncSession, field: str, filters: BaseModel = None):
        filter_dict = filters.model_dump(exclude_unset=True) if filters else {}
        logger.info(f"Поиск всех записей {cls.model.__name__}.{field} по фильтрам: {filter_dict}")
        try:
            query = select(getattr(cls.model, field)).filter_by(**filter_dict)
            result = await session.execute(query)
            records = result.scalars().all()
            logger.info(f"Найдено {len(records)} записей.")
            return records
        except SQLAlchemyError as e:
            logger.error(f"Ошибка при поиске всех записей {cls.model.__name__}.{field} по фильтрам {filter_dict}: {e}")
            raise
